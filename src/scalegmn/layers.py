...
class PositionalEncoding(nn.Module):
    ...
    def __init__(
        ...
        if self.different_linear:
            self.linears = nn.Parameter(
                torch.randn(
                    self.num_param_types, d_hid, d_hid if self.equiv_net else d_in
                )
            )
        ...
        if self.equiv_net:
            self.init_fn = EquivariantNet(
                ...
                ablate_symmetry=ablate_symmetry,
            )
    ...
    def apply_diff_layer(self, data):
        if self.param == "node":
            data.x = torch.bmm(
                data.x.unsqueeze(1),
                self.linears[data.node2type, :, :].transpose(-2, -1),
            ).squeeze(1)
            pos_embed = self.pos_embed[data.node2type]
        else:
            data.edge_attr = torch.bmm(
                data.edge_attr.unsqueeze(1),
                self.linears[data.edge2type, :, :].transpose(-2, -1),
            ).squeeze(1)
    ...
    def apply_pos_embed(self, data):
        if self.param == "node":
            if self.sum_pos_enc:
                ...
                if self.sum_on_io:
                    data.x = data.mask_hidden * self.transform(
                        data.x, data.node2type, "prod"
                    ) + (~data.mask_hidden) * self.transform(
                        data.x, data.node2type, "sum"
                    )
                else:
                    data.x = data.x * self.pos_embed[data.node2type]
            ...
            if self.sum_pos_enc:
                ...
                if self.sum_on_io and self.sum_on_io_edge:
                    ...
                    edge_mask = (
                        data.mask_hidden[data.edge_index[0]]
                        * ~data.mask_hidden[data.edge_index[1]]
                    ) + (
                        data.mask_hidden[data.edge_index[1]]
                        * ~data.mask_hidden[data.edge_index[0]]
                    )

                    data.edge_attr = edge_mask * self.transform(
                        data.edge_attr, data.edge2type, "prod"
                    ) + (~edge_mask) * self.transform(
                        data.edge_attr, data.edge2type, "sum"
                    )
                else:
                    data.edge_attr = data.edge_attr * self.pos_embed[data.edge2type]
            if self.final_linear_pos_embed:
        ...
        if self.param == "node":
            if self.sum_on_io:
                data.x = data.mask_hidden * self.init_fn(
                    ...
                    mask_hidden=(
                        data.mask_hidden if hasattr(data, "mask_hidden") else None
                    ),
                    mask_first_layer=(
                        data.mask_first_layer
                        if hasattr(data, "mask_first_layer")
                        else None
                    ),
            ...
            data.edge_attr = self.init_fn(
                ...
                mask_hidden=(
                    data.mask_hidden[data.edge_index[1]]
                    if hasattr(data, "mask_hidden")
                    else None
                ),
                mask_first_layer=(
                    data.mask_first_layer[data.edge_index[1]]
                    if hasattr(data, "mask_first_layer")
                    else None
                ),
            )
...
class InvariantLayer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ablate_symmetry = kwargs.get("ablate_symmetry", False)
        if final_mlp:
            self.phi = mlp(
                in_features=in_features + d_extra, out_features=out_features, **mlp_args
            )
        if sym == "sign" and sign_symmetrization:
            self.rho = mlp(
                in_features=in_features, out_features=in_features, **mlp_args
            )
...
class InvariantNet(nn.Module):
    ...
    def __init__(
        ...
        if self.equiv_on_hidden:
            if num_mlps in [2, 3]:
                self.phi_in = mlp(
                    in_features=d_in + d_extra, out_features=d_out, **mlp_args
                )
            if num_mlps == 3:
                self.phi_last = mlp(
                    in_features=d_in + d_extra, out_features=d_out, **mlp_args
    ...
    def forward(
        ...
        x_p = (
            torch.cat((x, extra_features), dim=-1) if extra_features is not None else x
        )
        if self.equiv_on_hidden:
            if self.num_mlps == 1:
                ...
                x = (
                    mask_hidden * x_canon
                    + mask_first_layer * self.phi_in(x_p)
                    + ((~mask_hidden) & (~mask_first_layer)) * self.phi_last(x_p)
                )
        else:
            x = x_canon
        return x
...
class MLPNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, extra_features=None, *args, **kwargs):
        r = (
            self.rho(x)
            if not self.pos_embed_dim
            else self.rho(torch.cat((x, extra_features), dim=1))
        )
        return r
...
class PermScaleInvariantReadout(nn.Module):
    ...
    def __init__(
        ...
        self.global_mlp = mlp(
            in_features=d_rho + d_rho * num_io_nodes, out_features=d_out, **mlp_args
        )
    ...
    def forward(self, x, batch, mask_hidden=None, sign_mask=None):
        ...
        i_o_nodes = to_dense_batch(
            x[~mask_hidden.squeeze(1)], batch[~mask_hidden.squeeze(1)]
        )[0]
...
class EquivariantNet(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_layers):
            layers.append(
                EquivariantLayer(
                    d_out if i else d_in,
                    d_out,
                    mlp_args,
                    d_extra,
                    symmetry=symmetry,
                    equiv_on_hidden=equiv_on_hidden
                    and not (layer_equiv_on_hidden or mlp_on_io),
                    num_mlps=num_mlps,
                    sign_symmetrization=sign_symmetrization,
                    ablate_symmetry=ablate_symmetry,
                )
        ...
        if self.mlp_on_io:
            ...
            self.eq_layers_in = nn.ModuleList(
                [
                    MLPNet(d_out if i else d_in, d_out, mlp_args, d_extra)
                    for i in range(n_layers)
                ]
            )
            if num_mlps == 3:
        ...
        if self.skip_connections:
            self.skip_layers = nn.ModuleList(
                [
                    nn.Linear(d_out if i else d_in, d_out, bias=False)
                    for i in range(n_layers)
                ]
            )
    ...
    def forward(
        ...
        for i, layer in enumerate(self.eq_layers):
            ...
            if self.layer_equiv_on_hidden or self.mlp_on_io:
                if self.num_mlps == 1:
                    x_tilde = mask_hidden * x_tilde_hid + (~mask_hidden) * layer(
                        x, extra_features=extra_features
                    )
                elif self.num_mlps == 2:
                    x_tilde = mask_hidden * x_tilde_hid + (
                        ~mask_hidden
                    ) * self.eq_layers_in[i](x, extra_features=extra_features)
                elif self.num_mlps == 3:
                    x_tilde = (
                        mask_hidden * x_tilde_hid
                        + mask_first_layer
                        * self.eq_layers_in[i](x, extra_features=extra_features)
                        + ((~mask_hidden) & (~mask_first_layer))
                        * self.eq_layers_last[i](x, extra_features=extra_features)
                    )
            else:
                x_tilde = x_tilde_hid
    ...
    def __repr__(self):
        ...
        layer_equiv_on_hidden_str = (
            f",\n  layer_equiv_on_hidden={self.layer_equiv_on_hidden}"
        )
        num_mlps_str = (
            f",\n  num_layer_types={self.num_mlps}"
            if self.layer_equiv_on_hidden
            else ""
        )
...
class EdgeUpdate(nn.Module):
    ...
    def __init__(
        ...
        self.sign_symmetrization = sign_symmetrization
        self.phi = mlp(
            in_features=(2 + (symmetry == "permutation")) * d_in,
            out_features=d_out,
            **mlp_args,
        )
        if symmetry != "permutation":
    ...
    def forward(self, x, edge_index, edge_attr, sign_mask=None):
        if self.symmetry == "permutation":
            h_e = self.phi(
                torch.cat((edge_attr, x[edge_index[0]], x[edge_index[1]]), dim=-1)
            )
        else:
            if self.symmetry == "hetero":
                x_canon_0 = sign_mask[edge_index[0]] * self.g_sign(x[edge_index[0]]) + (
                    ~sign_mask[edge_index[0]]
                ) * self.g_scale(x[edge_index[0]])
                x_canon_1 = sign_mask[edge_index[1]] * self.g_sign(x[edge_index[1]]) + (
                    ~sign_mask[edge_index[1]]
                ) * self.g_scale(x[edge_index[1]])

            else:
                x_canon_0 = self.g1(x[edge_index[0]])
                x_canon_1 = self.g1(x[edge_index[1]])
            h_e = self.W(edge_attr) * self.phi(
                torch.cat((x_canon_0, x_canon_1), dim=-1)
            )
        return h_e
