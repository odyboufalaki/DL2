...
def collect_latents(model, loader, device):
    """Return latents, labels, weights (list[tensor])."""
    zs, ys, wbs = [], [], []
    model.eval()
    for batch, wb in loader:
        batch = batch.to(device)
        zs.append(model.encoder(batch).cpu())
...
def get_args():
    ...
    p.add_argument("--conf", type=str, required=True, help="YAML config used at train")
    p.add_argument("--ckpt", type=str, required=True, help="Path to .pt model weights")
    p.add_argument(
        "--orbit_json",
        type=str,
        required=True,
