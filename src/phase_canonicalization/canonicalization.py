...
def store_canonicalized_params(w, b, paths, label, dataset_name):
    ...
    for datapoint in range(batch_size):
        ...
        if dataset_name != "labeled_mnist_inr":
            dict_of_tensors["label"] = (
                int(label[datapoint])
                if dataset_name == "labeled_fashion_mnist_inr"
                else label[datapoint]
            )
    ...
    for _split in splits:
        if extra_aug > 0 and conf["data"]["dataset"] == "cifar_inr":
            ...
            for i in range(extra_aug):
                aug_dsets.append(
                    dataset(
                        conf["data"], split="train", prefix=f"randinit_smaller_aug{i}"
                    )
                )
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            store_canonicalized_params(
                new_W, new_b, paths, data.label, conf["data"]["dataset"]
            )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--conf", type=str, required=True, default="canonicalization.yml"
    )
    ...
    arg_parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Reconstruct new INR, for testing purposes.",
    )
